...
TRAINING_CONFIG = SFTConfig(output_dir=str(OUTPUT_DIR), num_train_epochs=3, per_device_train_batch_size=8, gradient_accumulation_steps=2, learning_rate=2e-4, lr_scheduler_type="cosine", warmup_ratio=0.05, bf16=True, fp16=False, max_seq_length=2048, logging_steps=10, save_strategy="steps", save_steps=200, save_total_limit=3, eval_strategy="steps", eval_steps=200, optim="adamw_torch", weight_decay=0.01, dataset_text_field="text", packing=True, group_by_length=True)
...
def train():
    ...
    trainer = SFTTrainer(model=model, tokenizer=tokenizer, args=TRAINING_CONFIG, train_dataset=train_ds, eval_dataset=val_ds, peft_config=LORA_CONFIG)
